...
export const leaderboardRoutes = new Hono<HonoEnv>();
...
leaderboardRoutes.get("/", async (c) => {
  ...
  const allLogs = await prisma.sessionLog.findMany({ include: { user: true } });
  ...
  const weeklyByUser: Record<string, WeeklyLeader> = {};
  ...
  const monthlyByUser: Record<string, WeeklyLeader> = {};
  ...
  const weekChartData = weekDays.map((label, i) => {
    ...
    return { label, minutes: Math.round(dayLogs.reduce((s, l) => s + l.durationSeconds, 0) / 60) };
  });
  ...
  if (totalExercises > 100) {
    funFacts.push(`🎯 ${totalExercises.toLocaleString()} exercises completed across all sessions!`);
  }
  if (weeklyChampion) {
    funFacts.push(
  ...
  return c.json({
    ...
    allTimeTop,
    globalStats,
    chartData: { week: weekChartData, month: monthChartData, year: yearChartData },
    funFacts,
  });
});
